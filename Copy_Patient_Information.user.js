...
(function() {
    ...
    const validationRules = {
        ...
        notAcceptedInsurances: {
            '$empire y8e$': "ОШИБКА: Empire Y8E не берём в работу — нет читающих врачей.",
        },
        ...
        conditional: [
            ...
            },
            {
                ...
                excludeInsurance: 'empire plan by uhc',
                studyToReplace: 'Renal',
                newStudyName: '+Retro2',
                newDiagnosisCode: ['R10.84'],
            },
            ...
            { type: 'conflict',studies: [['ABD2','ABDO3','Ab2','Abdominal'],['Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2']],"message":"КОНФЛИКТ: Тесты ABD и Renal Bladder не могут быть вместе."},
            { type: 'conflict',studies: [['ABD2','ABDO3','Ab2','Abdominal'],['Abdominal Aorta2']],"message":"КОНФЛИКТ: Тесты ABD и AORTA не могут быть вместе."},
            { type: 'conflict',studies: [["Renal Doppler"],['Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2']],"message":"КОНФЛИКТ: Тесты REN DOPPLER и REN BLADDER не могут быть вместе."},
            ...
            { type: 'conflict',studies: [['Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2'],['PELV2','PEL2','Pelvic TV2','Scrotal','PEL TV']],"message":"КОНФЛИКТ: Тесты REN BLADDER и PELV не могут быть вместе."},
            { type: 'conflict',studies: [['Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2'],['Abdominal Aorta2']],"message":"КОНФЛИКТ: Тесты REN BLADDER и AORTA не могут быть вместе."},
        ...
        specificDoctorStudyRules: [
            {
                ...
                allowedStudies: ['ABD2', 'Ab2', 'Abdominal', 'PEL2', 'PELV2', 'PEL TV','Pelvic TV2','PELV3', 'Thyroid', 'Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2']
            },
        ],
        doctorRestrictions: {
            'Mittal, H.K.': { prohibits: ['ABD2', 'Ab2', 'Abdominal', 'PEL2', 'PELV2', 'PEL TV','Pelvic TV2','PELV3', 'Thyroid', 'Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2'] },
    ...
    const DEBUG = false; // включи вручную для отладки (в консоль уйдут данные пациента)
    const log = (...a) => { if (DEBUG) console.log(...a); };

    // Приоритет тестов: дорогой -> дешёвый. В конфликте оставляем более дорогой, дешёвый -> "next time".
    const STUDY_PRIORITY = [
        ['Carotid'],                                                              // CAR
        ['Echocardiogram'],                                                       // ECHO
        ['Renal Doppler','Retroperetonial2','Retroperetonial3','Retro','Renal','+Retro2'],  // REN
        ['LEA','LEA2'],                                                           // LEA
        ['LEV'],                                                                  // LEV
        ['Abdominal Aorta2'],                                                     // AOR
        ['UEA'],                                                                  // UEA
        ['ABD2','ABDO3','ABDO3cpt','Ab2','Abdominal','Cigna_ABD'],                // ABD
        ['PEL2','PELV2','PELV3','Pelvic TV2','Pel TV','Pel TA.TV','Scrotal'],     // PEL
        ['SUDO','SUDO3'],                                                         // SUDO
        ['Thyroid'],                                                              // THY
        ['Soft tissue'],                                                          // Soft tissue
    ];
    function studyPriority(studyName) {
        const s = (studyName || '').toUpperCase();
        for (let i = 0; i < STUDY_PRIORITY.length; i++) {
            if (STUDY_PRIORITY[i].some(n => n.toUpperCase() === s)) return i;
        }
        return 999; // неизвестный тест -> откладываем
    }

    // Англоязычные заметки для Copy answer: конфликты (что отложить) + повторы по истории
    function getAnswerNotes(extractedData, historyData) {
        const notes = [];
        const primaryIns = (extractedData['Insurance Subtype'] || '').toLowerCase();
        const allStudies = Object.keys(extractedData)
            .filter(k => /^Study\d+$/.test(k))
            .map(k => extractedData[k])
            .filter(s => s && s !== 'N/A');
        const strip = s => (s || '').replace(/\d/g, '').replace(/\s+/g, ' ').trim();

        // Конфликты: оставляем более дорогой тест, более дешёвый -> "next time"
        for (const rule of validationRules.conditional) {
            if (rule.type !== 'conflict') continue;
            if (!checkInsuranceMatch(rule.insurance, primaryIns)) continue;
            if (!checkStudyConditions(rule.studies, allStudies)) continue;
            const sides = (rule.studies || []).map(g => {
                const arr = Array.isArray(g) ? g : [g];
                return allStudies.find(s => arr.includes(s)) || arr[0];
            });
            if (sides.length >= 2 && sides[0] && sides[1]) {
                const [s1, s2] = sides;
                const keep  = studyPriority(s1) <= studyPriority(s2) ? s1 : s2;
                const defer = keep === s1 ? s2 : s1;
                notes.push(`${strip(defer)} next time, we can't do ${strip(keep)} and ${strip(defer)} in the same day.`);
            }
        }

        // Повторы: тест уже делали в пределах окна повтора
        const currentDOS = parseDate(extractedData['DOS']);
        if (historyData && historyData.length && currentDOS) {
            const key = Object.keys(validationRules.repeatInsuranceRules).find(k => {
                const kl = k.toLowerCase();
                return (kl.startsWith('$') && kl.endsWith('$')) ? kl.slice(1, -1) === primaryIns : primaryIns.includes(kl);
            });
            const cfg = validationRules.repeatInsuranceRules[key] || validationRules.repeatInsuranceRules['DEFAULT_DAYS'];
            const baseDays = (typeof cfg === 'object' && cfg.days) ? cfg.days : (typeof cfg === 'number' ? cfg : 180);
            const isHF = n => n.includes('healthfirst') || n.includes('hf ');
            for (const study of allStudies) {
                const days = (typeof cfg === 'object' && cfg.studyOverrides && cfg.studyOverrides[study]) || baseDays;
                for (const h of historyData) {
                    const histIns = h.insurances[0];
                    if (!histIns) continue;
                    if (!checkInsuranceMatch(primaryIns, histIns) && !(isHF(primaryIns) && isHF(histIns))) continue;
                    const hit = h.studies.find(hs => hs.replace(/\d/g, '').trim() === study.replace(/\d/g, '').trim());
                    if (!hit) continue;
                    const d = getDaysBetweenDates(currentDOS, parseDate(h.dos));
                    if (d !== null && d > 0 && d <= days) {
                        notes.push(`NO, ${strip(study)} was done ${d}d ago (${h.dos}).`);
                        break;
                    }
                }
            }
        }
        // Запрещённые тесты для этой страховки
        const findProhKey = (obj) => Object.keys(obj).find(k => {
            const kl = k.toLowerCase();
            if (kl.startsWith('$') && kl.endsWith('$')) return kl.slice(1, -1) === primaryIns;
            return primaryIns.includes(kl);
        });
        const prohKey = findProhKey(validationRules.prohibitedStudies);
        const prohList = prohKey ? validationRules.prohibitedStudies[prohKey] : [];
        for (const study of allStudies) {
            if (prohList.includes(study)) {
                notes.push(`NO, we can't do ${strip(study)} with this insurance.`);
            }
        }

        // UHC Dual Complete / UHC Community — текст про реферал
        if (checkInsuranceMatch(['uhc dual complete', 'uhc community'], primaryIns)) {
            const hasEcho = allStudies.some(s => (s || '').toLowerCase() === 'echocardiogram');
            const hasNonEcho = allStudies.some(s => (s || '').toLowerCase() !== 'echocardiogram');
            const refHikin = `We can only accept the patient after the referring office sends the referral with the following details:
...
Thank you for your understanding!`;
            const refMittal = `We can only accept the patient after the referring office sends the referral with the following details:
...
Thank you for your understanding!`;
            if (hasNonEcho) notes.push(refHikin);
            if (hasEcho) notes.push(refMittal);
        }
    ...
    // true только если для страховки есть ЯВНОЕ правило выбора врача (без дефолтного фолбэка)
    function insuranceHasDoctorRule(insName) {
        insName = (insName || '').toLowerCase();
        if (!insName || insName === 'n/a') return false;
        return Object.keys(validationRules.doctorInsuranceRestrictions).some(k => {
            const kl = k.toLowerCase();
            if (kl.startsWith('$') && kl.endsWith('$')) return kl.slice(1, -1) === insName;
            return insName.includes(kl);
        });
    }

    // Врачи, реально способные прочитать конкретный тест для страховки (Primary ∩ Secondary, минус запреты врача и спец-правила)
    function getAvailableReadingDoctors(primaryIns, secondaryIns, studyName) {
        const primaryAllowed = getAllowedDoctorsForIns(primaryIns);
        const secondaryAllowed = (secondaryIns && secondaryIns.length > 2 && secondaryIns !== 'n/a') ? getAllowedDoctorsForIns(secondaryIns) : null;
        let docs = secondaryAllowed ? primaryAllowed.filter(d => secondaryAllowed.includes(d)) : primaryAllowed;
        const studyUpper = (studyName || '').toUpperCase();
        docs = docs.filter(doctorName => {
            const rules = validationRules.doctorRestrictions[doctorName];
            if (!rules) return true;
            return !rules.prohibits.some(p => p.toUpperCase() === studyUpper);
        });
        docs = docs.filter(docName => passesSpecificDoctorStudyRules(docName, primaryIns, studyName));
        return docs;
    }

    function markRowSkipped(row, msg) {
        if (!row) return;
        row.style.backgroundColor = '#FFE8CC';
        const cell = row.querySelector('.x-grid3-td-8 .x-grid3-cell-inner');
        if (cell) cell.innerHTML = `<div style="font-weight:bold;font-size:11px;color:#B25900;">⏭️ ${escapeHtml(msg)}</div>`;
    }
...
const hipABD2CarotidSplit =
  ...
  return true;
        ...
        const notAcceptedKey = findRuleKey(validationRules.notAcceptedInsurances || {}, primaryInsuranceSubtype);
        if (notAcceptedKey) {
            result.errors.push(validationRules.notAcceptedInsurances[notAcceptedKey]);
        }
...
for (const historyEntry of historyData) {
            ...
            // В быстром Check: если для страховки нет ни одного врача под этот тест — ошибка (напр. Y8E + тест, который врач не читает)
            if (!checkDocuments) {
                const availDocs = getAvailableReadingDoctors(primaryInsuranceSubtype, secondaryInsurance, expectedStudyName);
                if (availDocs.length === 0) {
                    result.errors.push(`Study${i} ('${studyName}'): Нет читающих врачей для страховки "${primaryInsuranceSubtype}".`);
                }
            }
            ...
            if (!specificRuleApplied) {
                ...
                if (hasFinalReport) {
                    // Final Report готов: read уже не поменять, но врача не по страховке подсвечиваем (синим/warning)
                    if (reading && reading !== 'N/A' && reading !== '' && reading !== 'NOT ASSIGNED') {
                        const readingParts = reading.split(' / ').map(p => p.trim());
                        const doctorToValidate = readingParts.length > 1 ? readingParts[1] : readingParts[0];
                        ...
                        if (!primaryAllowed.some(doc => doctorToValidate.includes(doc))) {
                            result.warnings.push(`Study${i} ('${studyName}'): врач "${doctorToValidate}" не по Primary ("${primaryInsuranceSubtype}") — Final Report уже готов.`);
                        }
                        if (secondaryInsurance && secondaryInsurance.length > 2 && secondaryInsurance !== 'n/a') {
                            ...
                            if (!secondaryAllowed.some(doc => doctorToValidate.includes(doc))) {
                                result.warnings.push(`Study${i} ('${studyName}'): врач "${doctorToValidate}" не по Secondary ("${secondaryInsurance}") — Final Report уже готов.`);
                            }
                        }
                    }
                }
                    ...
                    expectedDoctors = expectedDoctors.filter(doctorName => {
                        ...
                        return !rules.prohibits.some(p => {
                            ...
                            return studyBaseName === rule; // точное совпадение, не подстрока
                        });
                    });
  ...
  passesSpecificDoctorStudyRules(docName, primaryInsuranceSubtype, expectedStudyName)
                    ...
                    let hint;
                    if (!insuranceHasDoctorRule(primaryInsuranceSubtype)) {
                        hint = `Врач НЕ НАЙДЕН для страховки "${extractedData['Primary insurance'] || primaryInsuranceSubtype}" — назначьте вручную.`;
                    } else {
                        hint = `Ожидаемые врачи: ${expectedDoctors.join(', ')}`;
  ...
  if (st === 'CAROTID' && !doctorToValidate.includes('Mittal')) {
    ...
    function displayValidationResult(row, { errors, warnings }) {
        ...
        const fmt = (m) => {
            ...
            return t === '' ? '(причина не указана — включите DEBUG)' : escapeHtml(t);
        };
        const allMessages = [
            ...errors.map(e => `<span style="color: #9B0000;">❌ ${fmt(e)}</span>`),
            ...warnings.map(w => `<span style="color: #0075b0;">⚠️ ${fmt(w)}</span>`)
        ];
        if (errors.length) log('Validation errors:', errors);
        if (errors.length > 0) {
            ...
            if (copyAllCell && !copyAllCell.querySelector('.copy-buttons-wrapper[data-source="helper"]')) {
                ...
                copyAnswerButton.addEventListener('click', async () => {
                    ...
                    if (extractedData['History URL']) {
                        try { historyData = await fetchHistoryData(extractedData['History URL']); } catch (e) {}
                    }
                    const notes = getAnswerNotes(extractedData, historyData);
                    let text = info + ' - ';
...
function selectBestDoctor(allowedDocs, studyName, insName) {
    ...
    const validDocs = allowedDocs.filter(docName => {
        ...
        return !restrictions.prohibits.some(p => {
            ...
            return sClean === pClean; // точное совпадение, не подстрока
            });
        });
            ...
            if (action.field === 'diag') {
                ...
                if (diagInput) {
                    ...
                    setTimeout(() => {
                         ...
                         function enterNextCode() {
                             ...
                             simulateKeyboardInput(diagInput, code, (success) => {
                                 if (!success) {
                                     showInterventionPrompt(`Не удалось ввести диагноз "${code}" для "${action.studyName}".\nПроверьте вручную и нажмите «Продолжить».`)
                                         .then(c => c === 'stop' ? stopAutoAssign() : setTimeout(enterNextCode, 500));
                                 } else {
                ...
                if (readingInput) {
                    simulateKeyboardInput(readingInput, action.value, (success) => {
                        if (!success) {
                            showInterventionPrompt(`Не удалось назначить Reading "${action.value}" для "${action.studyName}".\nПроверьте вручную и нажмите «Продолжить».`)
                                .then(c => c === 'stop' ? stopAutoAssign() : setTimeout(nextAction, 500));
                        } else {
...
function processCurrentPatient(expectedIdx, callback) {
    ...
    if (matchedSkip) {
        ...
        // Не берём в работу заблокированные страховки (напр. Y8E) — пропускаем с пометкой
        const primaryInsForBlock = (extractedData['Insurance Subtype'] || '').toLowerCase();
        const blockedKey = Object.keys(validationRules.notAcceptedInsurances || {}).find(k => {
            const kl = k.toLowerCase();
            return (kl.startsWith('$') && kl.endsWith('$')) ? kl.slice(1, -1) === primaryInsForBlock : primaryInsForBlock.includes(kl);
        });
        if (blockedKey) {
            markRowSkipped(row, validationRules.notAcceptedInsurances[blockedKey]);
            return callback(true);
        }
        ...
        // СТРОГО: если для страховки нет правила выбора врача — не назначаем дефолтного, пропускаем пациента
        const primaryInsForDoc = (extractedData['Insurance Subtype'] || '').toLowerCase();
        if (actions.some(a => a.field === 'reading') && !insuranceHasDoctorRule(primaryInsForDoc)) {
            markRowSkipped(row, `Для страховки "${extractedData['Primary insurance'] || primaryInsForDoc}" врач не найден — пропущено, назначьте вручную.`);
            console.warn('AutoAssign: пропуск пациента — нет врача для страховки:', primaryInsForDoc);
            return callback(true);
        }
...
function startAutoAssign() {
    ...
    function showInterventionPrompt(message) {
        return new Promise((resolve) => {
            const old = document.getElementById('magic-intervention');
            if (old) old.remove();
            const o = document.createElement('div');
            o.id = 'magic-intervention';
            o.style.cssText = `position:fixed; right:20px; bottom:20px; z-index:2147483647;
                background:#fff; border:2px solid #9B59B6; border-radius:8px; padding:14px 16px;
                box-shadow:0 4px 18px rgba(0,0,0,.25); max-width:340px; font-size:13px; color:#222;`;
            o.innerHTML = `<div style="font-weight:bold;color:#9B59B6;margin-bottom:6px;">⚠️ Нужно вмешательство</div>
                <div id="mi-msg" style="margin-bottom:10px;white-space:pre-wrap;"></div>
                <div style="display:flex;gap:8px;justify-content:flex-end;">
                  <button id="mi-stop" style="padding:5px 12px;border:1px solid #ccc;border-radius:5px;background:#dc3545;color:#fff;cursor:pointer;">Стоп</button>
                  <button id="mi-go" style="padding:5px 12px;border:1px solid #ccc;border-radius:5px;background:#2ECC71;color:#fff;cursor:pointer;">Продолжить</button>
                </div>`;
            o.querySelector('#mi-msg').textContent = message;
            document.body.appendChild(o);
            o.querySelector('#mi-go').onclick = () => { o.remove(); resolve('continue'); };
            o.querySelector('#mi-stop').onclick = () => { o.remove(); resolve('stop'); };
        });
    }
