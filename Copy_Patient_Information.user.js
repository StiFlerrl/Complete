...
(function() {
    ...
    function validatePatientData(extractedData, checkDocuments = true) {
        ...
        for (let i = 1; extractedData[`Study${i}`]; i++) {
            ...
            if (historyData.length > 0 && currentOrderDOS && validationRules.repeatInsuranceRules) {
                ...
                let studyRuleDays = insRuleKey ? validationRules.repeatInsuranceRules[insRuleKey] : (validationRules.repeatInsuranceRules['DEFAULT_DAYS'] || 90);
    ...
    function calculateRequiredActions(extractedData) {
        ...
        for (let i = 1; extractedData[`Study${i}`]; i++) {
            ...
            // диагноз
            const properDiag = validationRules.masterStudyList[studyName];
            if (properDiag) {
                ...
                if (!currentDiag || currentDiag === 'N/A' || currentDiag.trim() === '' || !isCorrect) {
                    actions.push({ studyName, field: 'diag', value: diagValues });
                }
            }
            ...
            if (!currentReading || currentReading === 'N/A' || currentReading === 'NOT ASSIGNED') {
                ...
                const specificRule = validationRules.specificReadingRules.find(rule => {
                    ...
                    const studyMatch = !rule.study || (Array.isArray(rule.study) ? rule.study.includes(studyName) : rule.study === studyName);
                ...
                const suitableDoc = selectBestDoctor(candidateDocs, studyName);
